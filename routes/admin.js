...
router.post('/create-mentor', async(req, res) => {
    try{
        ...
        res.send({message: "mentor create failed, Please enter correct details."})
    }
})
...
router.post('/create-student', async(req, res) => {
    try{
        ...
        if(existingStudent) {
            console.log("Student ID/Email/Phone already in data list...Please use another ID/email/phone number...");
...
router.post('/create-students/bulk', async (req, res) => {
    try {
        ...
        for (const batch of req.body) {
            ...
            for (const studentData of batch) {
                ...
                if (existingStudent) {
                    ...
                    console.log("Student Phone Number already used:", studentData.studentPhone);
                    // Log a message when the student already exists
                    console.log("Student data already added:", existingStudent);
                } else {
                    const newStudent = new StudentModel({ ...studentData, verified: true });
                    console.log("Bulk student data upload request received from admin, please wait few seconds...");
                    await newStudent.save();
        ...
        res.status(200).send({ message: "Bunch of students data successfully uploaded by admin.", bulkStudents });
...
router.put('/update-mentors', async (req, res) => {
    try {
        ...
        const result = await MentorModel.updateMany({}, { $set: {studentData: []} });

        if (result) {
            console.log("Mentor schema updated successfully.", result);
            res.status(200).json({ message: "Mentor schema updated successfully.", result });
        } else {
            console.log("Failed to update mentor schema. Result:", result);
            res.status(400).json({ message: "Failed to update mentor schema.", error });
        }
    } catch (error) {
...
router.put('/update-student-schema', async (req, res) => {
    try {
        ...
        if (result) {
            ...
            res.status(200).send({ message: "Students schema updated successfully.", result });
        } else {
            console.log("Failed to update students schema. Result:", result);
            res.status(400).json({ message: "Failed to update students schema.", error });
        }
    } catch (error) {
        console.log("Server Error:", error);
        res.status(500).json({ message: "Server accessing failed/Error", error });
...
const MAX_CAPACITY_PER_MENTOR = process.env.MAX_CAPACITY_PER_MENTOR || 10;
...
router.post('/previous-mentor', async (req, res) => {
    try {
        ...
        if (previousMentor) {
            ...
            const previousMentorName = previousMentor.mentor
            console.log("Previous mentor details:", previousMentorName);
            res.status(200).json({previousMentorName: previousMentorName});
        } else {
...
router.get('/previous-mentor/:id', async (req, res) => {
    try {
        ...
        const previousMentor = await StudentModel.findOne({ studentId: req.params.id});
...
router.get('/students-not-having-mentor', async (req, res) => {
    try {
        ...
        const studentsWithoutMentors = await StudentModel.find({ mentor: { $eq: null}});
        console.log(`Totally ${studentsWithoutMentors.length} students, not having mentor.`);
...
router.put('/student-update', async (req, res) => {
    try {
        ...
        const {studentId, studentName, studentEmail, studentPhone, batch, timing, course} = req.body;

        //this search can find particular student and update
        const updateStudent = await StudentModel.findOneAndUpdate( 
        {studentId: studentId}, {studentName,studentEmail, studentPhone, batch, timing, course}, {new: true});

        if (updateStudent) {
            console.log("Student data updated successfully.");
            console.log("Deleted details:", updateStudent);
            res.status(200).send({message: "Student data deleted successfully.", updateStudent});
        } else {
...
router.put('/mentor-update', async (req, res) => {
    try {
        ...
        const {mentorId, mentorName, mentorEmail, mentorPhone, batches, timing, course, studentData} = req.body;

        //this search can find particular mentor and update
        const updateMentor = await MentorModel.findOneAndUpdate( 
        {mentorId: mentorId}, {mentorId, mentorName, mentorEmail, mentorPhone, batches, timing, course, studentData}, {new: true});

        if (updateMentor) {
            console.log("Mentor data updated successfully.");
            console.log("Deleted details:", updateMentor);
            res.status(200).send({message: "Mentor data deleted successfully.", updateMentor});
        } else {
