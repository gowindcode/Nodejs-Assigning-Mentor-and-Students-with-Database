...
//this is we are using to get mentorData, because we stored mentor details in side array.
...
router.post('/login', async (req, res) => {
    try {
        ...
        if (studentDetails && loginDetails.studentEmail === studentDetails.studentEmail) {
...
//student can get details about his/her  via using email 
//here for get details we have student used id
// http://localhost:5000/student/status/23
...
router.get('/status/:id', async(req, res) => {
    try{
        ...
        const studentDetails = await StudentModel.findOne({studentId: req.params.id})
...
router.post('/request', async (req, res) => {
    try {
        ...
        const requestDetails = new RequestModel({...req.body, verified: true})
        ...
        const studentDetails = await StudentModel.findOne({
            ...
            mentorData: req.body.mentorData,
            verified: true
        })
        
        // Check if studentDetails is found our records and student id matches with the request id condition check
        // if (
        //     (studentDetails && requestDetails.studentId === studentDetails.studentId) 
        //     && (studentDetails && requestDetails.batch === studentDetails.batch)
        //     && arraysEqual(requestDetails.mentorData, studentDetails.mentorData))
            
            if (
                await studentDetails &&
                await requestDetails.studentId === studentDetails.studentId &&
                await requestDetails.batch === studentDetails.batch &&
                (await requestDetails.mentorData, studentDetails.mentorData)
            ){
            console.log("Student found.");
            ...
            console.log("Student present Batch records:", studentDetails.batch);
            console.log("Student request mentor:", requestDetails.mentorData);
            console.log("Student present mentor:", studentDetails.mentorData);
        
        
            await res.status(200).send({ message: "Student Request form successfully submitted", requestDetails });
        } else {
            console.log("Student not found or id does not match in our records")
            await res.status(404).send({ message: "Student request failed", studentDetails: null });
        }
    } catch (error) {
...
//         // Find the student in the database based on the provided student ID
//         const studentDetails = await StudentModel.findOne({
//             studentId: req.body.studentId,
//             verified: true
//         });
        
//         // Check if studentDetails is found in our records
//         if (studentDetails) {
//             console.log("Student found.");
...
//             console.log("Student Request form successfully submitted");
//             res.send({ message: "Student Request form successfully submitted", requestDetails });
//         } else {
//             console.log("Student not found in our records");
//             res.send({ message: "Student not found in our records", studentDetails: null });
//         }
//     } catch (error) {
//         console.error("Request form submission failed:", error);
//         res.status(500).json({ error: "Internal Server Error" });
